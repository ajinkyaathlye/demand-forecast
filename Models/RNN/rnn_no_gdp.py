...
table_df.drop(['Index', 'GDP', 'CPI'], inplace=True, axis=1)
table_df.index = pd.to_datetime(table_df.index)
table_df = table_df.groupby(['Date']).sum()
# table_df.reset_index(level=['CPI'], inplace=True)
#
...
def next_batch(training_data, steps):
    ...
    y_batch = np.array(training_data[rand_start:rand_start + steps + 1]).reshape(-1, 2)
    # print y_batch.shape
    return y_batch[:-1, :].reshape(-1, steps, 2), y_batch[1:, 0].reshape(-1, steps, 1)
...
test_set.drop(['Cost'], inplace=True, axis=1)
test_set.plot()
plt.show()